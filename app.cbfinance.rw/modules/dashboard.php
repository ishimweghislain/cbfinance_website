...
try {
    ...
    // 4. Total Revenue — 100% Ledger-Based for consistency across all reports
    $rev_res = $conn->query("SELECT SUM(credit_amount - debit_amount) as total 
                            FROM ledger 
                            WHERE SUBSTRING(account_code, 1, 1) = '4'");
    $rev_data = $rev_res->fetch_assoc();
    $stats['revenue']['total_revenue'] = floatval($rev_data['total'] ?? 0);
