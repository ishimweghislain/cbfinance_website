...
function buildPortfolioQuery($conn, $sd, $ed, $cf, $sf) {
    ...
    return "SELECT lp.*, c.customer_name, c.customer_code, c.phone, c.email, c.address,
        ...
        (SELECT IFNULL(SUM(balance_remaining), 0) FROM loan_instalments WHERE loan_id = lp.loan_id AND due_date < CURDATE()) as total_overdue_amount,
        ...
        (SELECT IFNULL(SUM(balance_remaining), 0) FROM loan_instalments WHERE loan_id = lp.loan_id) as live_total_outstanding,
